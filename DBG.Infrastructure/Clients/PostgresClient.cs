...
public class PostgresClient : IPostgresClient
{
    ...
    public PostgresClient(string address)
    {
    ...
    {
        if (await PingAsync() != true) throw new Exception("Cannot connect to Postgres worker.");
    }
    ...
    public async Task<int> ExecuteNonQueryAsync(Guid guid, string query, StringValues token,
        string database = "postgres")
    {
        await CheckConnectivityAsync();
        _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
